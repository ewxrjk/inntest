...
def test_list_wildmat(hierarchy=None):
    """inntest.Tests.test_list_wildmat()
...
def test_list_wildmat_single(hierarchy=None):
    """inntest.Tests.test_list_wildmat_single()
...
def _test_list(conn, kw, wildmat=None):
    """inntest.Tests._test_list(CONN, KW, [WILDMAT])
    ...
    regex=getattr(inntest.Tests, regex_name, None)
    ...
    method=getattr(inntest.Tests, method_name, None)
    if method is not None:
        return method(lines, kw)

def _check_list_overview_fmt(lines, kw):
    """inntest.Tests._check_list_overview_fmt(LINES, KW)
...
def _check_list_motd(lines, kw):
    """inntest.Tests._check_list_motd(LINES, KW)
...
def _wildmat_pattern_to_re(pattern):
    """inntest.Tests._wildmat_pattern_to_re(PATTERN) -> RE
...
def _wildmat_to_function(wildmat):
    """inntest.Tests._wildmat_to_function(WILDMAT) -> FUNCTION
